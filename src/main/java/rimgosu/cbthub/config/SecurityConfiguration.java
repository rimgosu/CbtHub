...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        ...
        http.authorizeHttpRequests() //사용자의 요청을 핸들링
                .antMatchers("/","/members/**").permitAll() //  "/", "member"하위에 모든 접근을 허용하겠다.
                .antMatchers("/board/**").authenticated()  // board로 접근하는 모든 경우는 인증된(로그인한) 사용자만 허용됨.
                .and()  //추가
                .formLogin() //우리가 만든 별도의 로그인 폼을 사용하겠다.
                .loginPage("/members/login")//로그인 페이지는 member안에 login에서 하겠다.
                .defaultSuccessUrl("/") //로그인 성공시 board list로 이동하겠다.
                .and() // 추가
                .logout()
                .logoutUrl("/members/logout") // 로그아웃 실행하고 싶다면 member/logout으로 요청하겠다.
